...
func _start_test_mode() -> void:
	...
	var scene_path := "res://addons/agentic_godot_validation/runtime/scenes/test_bootstrap.tscn"
	var bootstrap: PackedScene = load(scene_path)

	if bootstrap == null:
		push_error("Test mode requested but the validation addon is missing. Run setup.ps1 first.")
		return
