...
| `src/tools/` | Repo-owned checks: `check_scripts.ps1` (compile gate, engine half in `compile_check.gd`), `lint.ps1`. `presskit/cover.gd` composes a store cover out of captured stills — game-agnostic, every colour and crop a flag; the capture tool beside it is written per game (press-kit skill). |
